...
import { ChevronDown, ChevronUp, Sparkles, ArrowRight, Flame, Flower2 } from "lucide-react";
...
export default function ServicesClient({ services, poojas, consultations }: ServicesClientProps) {
  // Category accordion state
  const [isHomasOpen, setIsHomasOpen] = useState(true);
  const [isPoojasOpen, setIsPoojasOpen] = useState(true);
    ...
    <div className="space-y-12 max-w-7xl mx-auto">
      ...
      <div id="homas" className="bg-white rounded-3xl border border-amber-200/80 shadow-md overflow-hidden transition-all scroll-mt-24">
        ...
        >
          ...
          <div className="w-10 h-10 rounded-xl bg-amber-200/80 border border-amber-400/20 flex items-center justify-center text-amber-900 group-hover:bg-amber-500/20 transition-colors shrink-0">
      ...
      {/* 2. SACRED POOJAS & HOLY PARAYANAS SECTION */}
      <div id="poojas" className="bg-white rounded-3xl border border-amber-200/80 shadow-md overflow-hidden transition-all scroll-mt-24">
        {/* Accordion Heading Bar */}
        <button
          onClick={() => setIsPoojasOpen(!isPoojasOpen)}
          className="w-full p-6 sm:p-8 bg-gradient-to-r from-amber-100 via-orange-50 to-amber-100 text-slate-900 border-b border-amber-200/80 flex items-center justify-between gap-4 text-left transition-colors group"
        >
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-amber-500/20 border border-amber-400/30 flex items-center justify-center shrink-0">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold tracking-widest uppercase text-amber-900/90 bg-amber-400/10 px-2.5 py-0.5 rounded-full border border-amber-400/20">
                  Devata Upasana &amp; Vratha
                </span>
                <span className="text-xs text-slate-600 font-mono">({poojas.length} Poojas)</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-serif font-bold text-amber-950 mt-1">
                Sacred Poojas &amp; Holy Parayanas
              </h2>
            </div>
          </div>
          <div className="w-10 h-10 rounded-xl bg-amber-200/80 border border-amber-400/20 flex items-center justify-center text-amber-900 group-hover:bg-amber-500/20 transition-colors shrink-0">
            {isPoojasOpen ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
          </div>
        </button>
              ...
              {poojas.map((item) => (
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div>
                      <span className="text-[10px] font-bold text-amber-900 uppercase tracking-wider bg-amber-100/70 px-2.5 py-1 rounded-md border border-amber-200">
                        Vedic Pooja
                      </span>
                      <h3 className="text-lg font-serif font-bold text-slate-900 mt-2 group-hover:text-amber-800 transition-colors">
                        {item.title}
      ...
      {/* 3. ASTROLOGY CONSULTATIONS SECTION */}
      <div id="consultations" className="bg-white rounded-3xl border border-amber-200/80 shadow-md overflow-hidden transition-all scroll-mt-24">
        {/* Accordion Heading Bar */}
        <button
